...
jest.mock('@opentelemetry/api', () => ({
  trace: { getTracer: jest.fn(() => ({ startSpan: jest.fn(() => ({ setAttributes: jest.fn(), setStatus: jest.fn(), recordException: jest.fn(), end: jest.fn() })) })) },
...
import { PromptRunnerService, LLMProviderService } from '@kodus/kodus-common/llm';
...
describe('GitHubPullRequestHandler - KodyRules sync on PR merged', () => {
  ...
  let runCodeReviewAutomationUseCase: { execute: jest.Mock; findTeamWithActiveCodeReview: jest.Mock };
  ...
  const organizationAndTeamData = { organizationId: 'org-1', teamId: 'team-1' } as any;
  const repository = { id: '1034598462', name: 'bug-game-running', fullName: 'kodustech/bug-game-running' } as any;
  const prNumber = 7;

  beforeEach(async () => {
    logger = { log: jest.fn(), error: jest.fn(), warn: jest.fn(), debug: jest.fn(), info: jest.fn() } as any;
    savePullRequestUseCase = { execute: jest.fn().mockResolvedValue({}) };
    runCodeReviewAutomationUseCase = {
      execute: jest.fn(),
      findTeamWithActiveCodeReview: jest.fn().mockResolvedValue({ organizationAndTeamData }),
    };
    ...
    codeManagementService = {
      ...
      getPullRequestByNumber: jest.fn().mockResolvedValue({ head: { ref: 'feat/new_rules_4' }, base: { ref: 'main' } }),
      getRepositoryContentFile: jest.fn().mockImplementation(async ({ file }: any) => {
        ...
        return { data: { content: Buffer.from(content, 'utf-8').toString('base64'), encoding: 'base64' } };
      }),
    } as any;
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        // Real LLM services (no LLMModule to avoid DI useExisting constraints)
        PromptRunnerService,
        LLMProviderService,
        { provide: 'LLM_LOGGER', useValue: logger },
        { provide: SavePullRequestUseCase, useValue: savePullRequestUseCase },
        { provide: RunCodeReviewAutomationUseCase, useValue: runCodeReviewAutomationUseCase },
        { provide: ChatWithKodyFromGitUseCase, useValue: chatWithKodyFromGitUseCase },
        { provide: CodeManagementService, useValue: codeManagementService },
        { provide: GenerateIssuesFromPrClosedUseCase, useValue: generateIssuesFromPrClosedUseCase },
        { provide: CreateOrUpdateKodyRulesUseCase, useValue: upsertRuleUseCase },
      ],
    }).compile();
  ...
  it('deve criar Kody Rules a partir de arquivos de rules ao mergear PR no default branch, preenchendo sourcePath e path', async () => {
    const params = {
      ...
      payload: {
        ...
        pull_request: {
          ...
          merged: true,
          base: { ref: 'main' },
          html_url: 'https://github.com/kodustech/bug-game-running/pull/7',
        },
    ...
    // upsert deve ser chamado uma vez por arquivo mapeado
    expect(upsertRuleUseCase.execute).toHaveBeenCalled();
    const calls = upsertRuleUseCase.execute.mock.calls;
    expect(calls.length).toBe(3);

    const payloads = calls.map((c: any[]) => c[0]);
    // valida fields essenciais
    for (const dto of payloads) {
      expect(dto.title).toBe('Rule from file');
      expect(dto.rule).toBeTruthy();
      expect(dto.repositoryId).toBe(repository.id);
      expect(dto.severity).toBe('medium');
      expect(dto.origin).toBe('user');
      expect(dto.scope).toBe('file');
      expect(dto.path).toMatch(/\.cursor\/rules\/.+\.mdc$/);
      expect(dto.sourcePath).toEqual(dto.path); // fallback para o próprio filename
    }

    // valida que buscamos conteúdo via PR refs
    expect(codeManagementService.getPullRequestByNumber).toHaveBeenCalledWith(
      expect.objectContaining({ prNumber })
    );
    expect(codeManagementService.getRepositoryContentFile).toHaveBeenCalled();
  });
});
