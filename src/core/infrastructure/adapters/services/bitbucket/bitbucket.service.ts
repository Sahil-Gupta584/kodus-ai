    ...
    */
    ...
    async getDiffForFile(params: {
        ...
        try {
            ...
            const baseCommits = await bitbucketAPI.commits
                .list({
                    repo_slug: `{${repository.id}}`,
                    workspace: `{${workspace}}`,
                })
            ...
            if (!baseCommits || baseCommits.length === 0) {
                this.logger.warn({
                    message: `No base commits found for repository ${repository.name}`,
                    context: BitbucketService.name,
                    metadata: {
                        organizationAndTeamData,
                        repository,
            ...
            const pullCommits = await bitbucketAPI.pullrequests
                .listCommits({
                    repo_slug: `{${repository.id}}`,
                    workspace: `{${workspace}}`,
                    pull_request_id: prNumber,
                ...
                .then((res) =>
                    this.getPaginatedResults<Schema.BaseCommit>(
                        bitbucketAPI,
                        res,
                    ),
                );
            ...
            if (!pullCommits || pullCommits.length === 0) {
                this.logger.warn({
                    message: `No commits found for pull request #${prNumber} in repository ${repository.name}`,
                    context: BitbucketService.name,
                    metadata: {
                        organizationAndTeamData,
                        repository,
            ...
            const lastBaseCommit = baseCommits[0];
            const lastPullCommit = pullCommits[0];

            const diff = await bitbucketAPI.commits.getDiff({
                repo_slug: `{${repository.id}}`,
                workspace: `{${workspace}}`,
                spec: `${lastPullCommit.hash}..${lastBaseCommit.hash}`,
                path: filePath,
            });
            ...
            this.logger.error({
                message: `Error retrieving diff for file ${filePath} in pull request #${prNumber}`,
                context: BitbucketService.name,
                serviceName: 'BitbucketService getDiffForFile',
                error: error,
                metadata: {
                    organizationAndTeamData: params.organizationAndTeamData,
