...
} from '@/shared/utils/langchainCommon/prompts/codeReview/codeReview';
import { prompt_potentialIssues } from '@/shared/utils/langchainCommon/prompts/codeReview/categories/potentialIssues';
import { prompt_security } from '@/shared/utils/langchainCommon/prompts/codeReview/categories/security';
import { prompt_errorHandling } from '@/shared/utils/langchainCommon/prompts/codeReview/categories/errorHandling';
import { prompt_refactoring } from '@/shared/utils/langchainCommon/prompts/codeReview/categories/refactoring';
import { prompt_performanceOptimization } from '@/shared/utils/langchainCommon/prompts/codeReview/categories/performanceOptimization';
import { prompt_maintainability } from '@/shared/utils/langchainCommon/prompts/codeReview/categories/maintainability';
import { prompt_codeStyle } from '@/shared/utils/langchainCommon/prompts/codeReview/categories/codeStyle';
import { prompt_documentationComments } from '@/shared/utils/langchainCommon/prompts/codeReview/categories/documentationComments';
...
export class LLMAnalysisService implements IAIAnalysisService {
    ...
    public async specificCategoriesCodeReview(
        organizationAndTeamData: OrganizationAndTeamData,
        prNumber: number,
        fileContext: FileChangeContext,
        reviewModeResponse: ReviewModeResponse,
        ...
        try {
            ...
            if (activeCategories.length === 0) {
                this.logger.log({
                    message: `No active categories found for PR#${prNumber}`,
                    context: LLMAnalysisService.name,
                });
                return [];
            }

            // Executar análise para cada categoria ativa em paralelo
            const analysisResults = await Promise.all(
                activeCategories.map((category) =>
                    this.processSpecificCategory(
                        organizationAndTeamData,
                        prNumber,
                        fileContext,
                        reviewModeResponse,
            ...
            this.logger.error({
                message: `Error generating suggestions for multiple categories for PR#${prNumber}`,
                error,
                context: LLMAnalysisService.name,
            });
            return [];
    ...
    private async processSpecificCategory(
        organizationAndTeamData: OrganizationAndTeamData,
        prNumber: number,
        fileContext: FileChangeContext,
        reviewModeResponse: ReviewModeResponse,
        ...
        try {
            ...
            const chain = await this.createCategoryCodeReviewChainWithFallback(
                provider,
                baseContext,
                reviewModeResponse,
                category,
            ...
            //analysisResult.categoryName = category;

            return analysisResult;
        } catch (error) {
            this.logger.error({
                message: `Error generating suggestions for category '${category}' for PR#${prNumber}`,
                error,
                context: LLMAnalysisService.name,
            });
            return null;
    ...
    private getCategoryPrompt(category: string): string {
        const categoryPromptMap = {
            ...
            error_handling: prompt_errorHandling,
            refactoring: prompt_refactoring,
            performance_and_optimization: prompt_performanceOptimization,
            maintainability: prompt_maintainability,
            potential_issues: prompt_potentialIssues,
            code_style: prompt_codeStyle,
            documentation_and_comments: prompt_documentationComments,
        };

        if (!categoryPromptMap[category]) {
            this.logger.warn({
                message: `No prompt function found for category '${category}'`,
                context: LLMAnalysisService.name,
            });
            return null;
        }

        return categoryPromptMap[category]();
    }

    private getActiveCategories(reviewOptions: any): string[] {
        if (!reviewOptions) return [];

        return Object.keys(reviewOptions).filter(
            (key) =>
                // Filtrar apenas as categorias válidas (excluir kody_rules e breaking_changes)
                reviewOptions[key] === true &&
                key !== 'kody_rules' &&
                key !== 'breaking_changes',
        );
    }

    private async createCategoryCodeReviewChainWithFallback(
        provider: LLMModelProvider,
        context: any,
        reviewMode: ReviewModeResponse,
        category: string,
        ...
        try {
            const mainChain = await this.createCategoryCodeReviewProviderChain(
                provider,
                reviewMode,
                context,
                category,
            ...
            const fallbackChain =
                await this.createCategoryCodeReviewProviderChain(
                    fallbackProvider,
                    reviewMode,
                    context,
                    category,
            ...
            return mainChain
                ...
                .withConfig({
                    runName: `generateSuggestionsFor${category}`,
                    metadata: {
                        organizationId:
                            context?.organizationAndTeamData?.organizationId,
                        teamId: context?.organizationAndTeamData?.teamId,
            ...
            this.logger.error({
                message: `Error creating analysis chain with fallback for category '${category}'`,
                error,
                context: LLMAnalysisService.name,
                metadata: {
                    provider,
    ...
    private async createCategoryCodeReviewProviderChain(
        provider: LLMModelProvider,
        reviewModeResponse: ReviewModeResponse,
        context: any,
        category: string,
        ...
        try {
            ...
            if (!categoryPrompt) {
                throw new Error(
                    `Failed to get prompt for category '${category}'`,
                );
            }
            ...
            const payload: NewCodeReviewPayload = {
                ...
                categorySpecificInstructions: categoryPrompt,
            ...
            this.logger.error({
                message: `Error creating analysis code chain for category '${category}'`,
                error,
                context: LLMAnalysisService.name,
